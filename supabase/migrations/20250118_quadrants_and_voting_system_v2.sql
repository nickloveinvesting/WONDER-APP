-- PONDER QUADRANTS & VOTING SYSTEM (v2 - Handles existing enum)
...
-- Quadrant enum already exists, skip if error
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'quadrant_type') THEN
...
DROP POLICY IF EXISTS "Users can insert own post votes" ON post_votes;
DROP POLICY IF EXISTS "Users can insert own comment votes" ON comment_votes;
DROP POLICY IF EXISTS "Users can update own post votes" ON post_votes;
DROP POLICY IF EXISTS "Users can update own comment votes" ON comment_votes;
DROP POLICY IF EXISTS "Users can delete own post votes" ON post_votes;
